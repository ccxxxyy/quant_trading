...
[![Version](https://img.shields.io/badge/version-0.1.0-blue.svg)](https://github.com/ccxxxyy/quant_trading)
[![CI](https://github.com/ccxxxyy/quant_trading/actions/workflows/ci.yml/badge.svg)](https://github.com/ccxxxyy/quant_trading/actions/workflows/ci.yml)
[![Ruff](https://img.shields.io/badge/code%20style-ruff-261230?logo=ruff)](https://docs.astral.sh/ruff/)
[![Data](https://img.shields.io/badge/data-AkShare%20%7C%20yfinance-brightgreen.svg)](https://github.com/akfamily/akshare)
[![License](https://img.shields.io/badge/license-personal%20use-lightgrey.svg)](https://github.com/ccxxxyy/quant_trading)

一个**事件驱动**、**模块化**、支持**多市场**（A股/美股/期货/场内外基金）和**多策略**的 Python 量化交易平台。

> **什么是量化交易？**
> 量化交易是指用数学模型和计算机程序来代替人工判断，自动分析行情数据、发出买卖信号并执行交易。

## 目录

1. [Web 仪表盘介绍](#web-仪表盘介绍)
2. [系统架构](#系统架构)
3. [功能详解](#功能详解)
   1. [数据获取与管理](#1-数据获取与管理)
   2. [策略回测](#2-策略回测)
   3. [七种内置交易策略](#3-七种内置交易策略)
   4. [策略参数优化](#4-策略参数优化)
   5. [风险控制](#5-风险控制)
   6. [执行算法（大单拆分）](#6-执行算法大单拆分)
   7. [投资组合管理](#7-投资组合管理)
   8. [实盘网关](#8-实盘网关)
   9. [AI/机器学习模块](#9-ai机器学习模块)
   10. [监控与告警](#10-监控与告警)
   11. [Web 仪表盘](#11-web-仪表盘)
   12. [命令行工具](#12-命令行工具)
...
12 个页面的 Web 管理界面，浏览器即可完成数据、回测、模拟盘与风控操作。
...
或：`uv run quant-web` → 打开 **http://127.0.0.1:8888**

| 页面 | 做什么 |
|------|--------|
| **总览** | 标的数 / 策略数 / 默认资金 / 交易所；最近一次回测权益曲线 |
| **数据管理** | AkShare / yfinance 拉行情；三栏已存储标的；K 线预览；数据目录 + SQL |
| **回测实验室** | 选策略与参数；默认初始资金 5000、下单数量 60；复权 / T+1 / 过户费；权益与回撤曲线；策略对比；Walk-Forward；蒙特卡洛 |
| **参数优化** | 网格搜索参数，按 Sharpe 排名 |
| **监控告警** | 阈值、告警列表、Webhook/邮件推送 |
| **模拟盘** | 多账户、下单（含追踪止损/条件单）、TWAP/VWAP 预览、网关连接 |
| **风控中心** | 冻结/清仓、仓位管理、黑名单、流动性过滤、风控日报 |
| **实时策略** | 策略启停、WebSocket 行情接入、活跃订单 |
| **AI 实验室** | 7 个内置因子、特征预览、LightGBM 说明、因子缓存（需 `uv sync --extra ml`） |
| **策略库** | 内置策略卡片 + 配对分析（相关/协整/对冲比） |
| **运维中心** | 定时任务、进程守护 |
| **系统设置** | 可视化改 YAML 配置 |

默认使用**真实本地行情**（演示数据勾选框默认关闭）。无数据时请先到「数据管理」拉取，或临时勾选演示模式。

CLI 回测示例：

```bash
uv run quant backtest dual_ma --symbol 008888.OTC --start 2024-01-01
```
...
- **AkShare**：获取 A 股（上交所 SSE、深交所 SZSE）、中国期货（中金所、上期所、大商所、郑商所）、场外开放式基金（OTC）的日线数据。无需注册，直接调用。
- **yfinance**：获取美股（纳斯达克、纽交所）及全球市场的行情数据。同样无需注册。

> **场外基金回测说明**：系统通过 AkShare 获取基金累计净值走势，将净值转换为 OHLC 格式用于策略回测。使用方法：在标的代码栏输入 `基金代码.OTC`（如 `161725.OTC` = 招商中证白酒），其他操作与股票一致。
...
可以自己编写新策略（继承 `BarSeriesStrategy` 基类即可），然后放到 `strategies/` 目录下使用。

<a id="4-策略参数优化"></a>
### 4. 策略参数优化

同一个策略使用不同的参数（比如"均线天数设为 5 天"还是"20 天"），回测效果会差异很大。**参数优化器**可以自动遍历所有参数组合，帮忙找到历史表现最好的参数设置。

例如：双均线策略的快线周期从 5 到 20、慢线周期从 20 到 60，系统会自动跑完所有组合（如 5+20, 5+25, 5+30, ..., 20+60），然后按夏普比率排名告诉哪个组合最好。
...
3. **日亏损检查**：当日累计亏损超过总资金的 5% 时，自动停止交易
...
- **TWAP**（时间加权）：把大单均匀分成 N 份，每隔一段时间下一份
...
代码层已实现，Web「AI 实验室」可操作因子计算与缓存；完整训练/预测需安装 ml 依赖。

| 能力 | 是否已实现 | 说明与使用思路 |
|------|------------|----------------|
| **特征工程引擎** | ✅ 已实现 | `FeatureEngine` 内置 7 个因子：动量×3、波动率×2、RSI、量比；Web 可计算预览并缓存到 Parquet |
| **LightGBM 模型** | ✅ 代码已实现 | `LightGBMModel` 可 fit/predict/save/load；需安装 `[ml]`；适合用因子表预测未来收益 |
| **预测服务** | ✅ 代码已实现 | `PredictService`：加载模型 → 算特征 → 输出信号；适合接到自定义策略或脚本，非开箱即用的“一键实盘 AI” |
| **Walk-Forward** | ✅ 已实现（策略向） | Web/API 对**交易策略**做滚动训练窗/测试窗回测，防过拟合；不是每次窗口自动重训 LightGBM |

**有没有必要用 AI？**  
日频基金/股票信号，规则策略（双均线、海龟等）通常更透明、更好调试。AI 更适合：已有大量样本、愿意做特征与样本外验证、把模型当研究工具。个人账户若数据短、标的少，优先把回测资金/仓位配准，再考虑 ML。

**推荐实现路径：** 拉真实日线 → AI 实验室算因子并缓存 → 脚本用 `PredictService.train` 训练 → Walk-Forward / 样本外检验 → 再考虑接入策略。
...
- **AkShare** → A 股 / 指数 / 期货 / 可转债 / 场外基金（`.OTC`）
...
| **标的** | Instrument | 你要交易的东西，可以是股票、期货合约、ETF基金、场外基金等 |
| **交易所** | Exchange | 买卖证券/期货的市场，如上交所(SSE)、深交所(SZSE)、纳斯达克(NASDAQ)、场外(OTC) |
| **场外基金** | OTC Fund | 通过基金公司或第三方平台申赎的开放式基金，代码格式如 `161725.OTC` |
| **场内基金** | ETF | 在证券交易所上市交易的基金，代码格式如 `510300.SSE`，像股票一样买卖 |
