...
- **模拟盘**：账户 KPI（总资产/可用资金/手续费/持仓数）+ 下单面板（标的、方向、类型含**追踪止损/条件单**、数量、价格） + 持仓表格 + 挂单列表 + **TWAP/VWAP 执行算法拆单预览**（选择算法、切片数、间隔 → 预览拆单方案）+ **券商网关连接面板**（模拟盘/CTP/IB 网关状态展示与一键连接），支持重置模拟盘
...
- **AI 实验室**：展示 7 个内置 Alpha 因子列表（动量/波动率/RSI/量比）、可用 ML 模型卡片（LightGBM）、特征计算预览（输入标的 → 计算全部因子 → 展示数据表格）
