...
- **真实交易成本**：佣金（默认万三）+ 滑点（默认万一）+ 印花税（卖出时万五），完整模拟 A 股费用结构
...
系统自带一个专业级的 Web 管理界面（深色主题交易终端风格），包含 **11 个页面**，**所有功能均可在浏览器中完成操作**：

- **总览页**：四个核心指标卡片（已存储标的数、内置策略数、默认资金、支持交易所数），点击可跳转到对应管理页面
- **数据管理**：选择数据源和标的代码，一键拉取行情数据，查看 K 线图预览
- **回测实验室**：选择策略、设置参数和资金，**页面内提供复权模式下拉（不复权/前复权/后复权）和 T+1 开关**，勾选即可模拟 A 股交易规则。回测后查看权益曲线、回撤图、交易记录和绩效报告，还可一键"策略对比"
- **参数优化**：选择策略 → 自动生成参数搜索范围（支持逗号分隔编辑） → 网格搜索所有组合 → 按 Sharpe 降序排名展示前 30 名，顶部 KPI 显示最优 Sharpe/收益率/组合总数
- **监控告警**：显示告警统计 KPI（总数/严重/警告/信息）、阈值配置（回撤、日亏损、持仓比例、成交偏离）、告警记录表格（时间、级别徽章、类型、消息），支持刷新告警和发送测试告警
- **模拟盘**：账户 KPI（总资产/可用资金/手续费/持仓数）+ 下单面板（标的、方向、类型、数量、价格） + 持仓表格 + 挂单列表，支持重置模拟盘（自定义初始资金和费率）
- **风控中心**：四个实时状态 KPI（冻结状态/策略状态/当日盈亏/下单笔数）+ 四个紧急操作按钮（冻结/解冻/暂停策略/恢复策略）+ 一键清仓按钮 + 风控详情面板。异常时一键操作，无需敲命令
- **实时策略**：选择策略和标的后一键启动，实时显示运行状态（策略名/绑定标的/已接收K线数）。内置"手动推送K线"面板用于测试，推送记录实时滚动展示。一键停止运行
