...
- **风控中心**：四个实时状态 KPI（冻结状态/策略状态/当日盈亏/下单笔数）+ 四个紧急操作按钮（冻结/解冻/暂停策略/恢复策略）+ 一键清仓按钮 + 风控详情面板 + **浮亏自动减仓**（设定触发阈值和减仓比例，系统检测并生成减仓单）+ **仓位管理器**（等权 / 风险平价 / 凯利公式 / 固定金额四种模式，自动计算各标的目标仓位）+ **事前四重风控规则**（展示单标的持仓/单笔下单/下单频率/日亏损四道检查的当前阈值，支持在线修改实时生效）
